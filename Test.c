...
*In this assignment I am able to use structures and unions
*The update that I have done is with the complete password and applied structures and unions.
**********************************************************************************************************/
...
struct Login //using structure to change the password code completely
{
	char Password[30];
	char Username[30];
};
...
/***********************************************************************************************************
* getUserInfoAndVerify() function
* This function gets the username and password then compares it and allows the user to continue on
***********************************************************************************************************/
void getUserInfoAndVerify()
{
	union User user1;

	// create get functions//
	printf("Enter Username :");
	fgets(user1.l1.Username, 30, stdin);
	printf("%s", user1.l1.Username);
	printf("\n");
	printf("Enter Password? :");
	fgets(user1.l1.Password, 30, stdin);
	printf("%s", user1.l1.Password);
	printf("\n");

	//declared variables for verification
	char Diancastor[30];
	char Security78[30];

	// get username and password from user
	printf("\nEnter your Username :\n");
	fgets(Diancastor, 30, stdin);
	printf("\nEnter your Password :\n");
	fgets(Security78, 30, stdin);

	//declared variables for validation
	int u = 0;
	int p = 0;

	if (strcmp(user1.l1.Username, Diancastor) == 0)
	{
		u = 1;
		printf("\nTEST: Username matches");

		if (strcmp(user1.l1.Password, Security78) == 0)
		{
			p = 1;
			printf("\nTEST: Password matches");// making sure that password matches
		}
		else
		{
			printf("\nIncorrect Password");
...
{
	introPrinter();
	while (GlobalVerified != 1)
	{
		getUserInfoAndVerify();
	}
